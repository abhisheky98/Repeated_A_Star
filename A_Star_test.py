...
def a_star(dim, P, grid, heu):
    ...
    while len(fringe)!=0:
        ...
        if (i,j) == (dim-1,dim-1):
            ...
            break
        #print((i,j))
        if i-1 >= 0 and grid[i-1][j] != 1 and p[i][j] != (i-1,j):
            if p[i-1][j] != -1:
                if g[i][j]+1+h[i-1][j]<f[i-1][j]:
                    delete_fringe((i-1,j),fringe)
            if (p[i-1][j] == -1) or (p[i-1][j] !=-1 and g[i][j]+1+h[i-1][j]<f[i-1][j]):
                p[i-1][j]=(i,j)
                g[i-1][j]=g[i][j]+1
                f[i-1][j]=g[i-1][j]+h[i-1][j]
                insert_fringe(i-1,j,fringe,f)
            
        if j+1 < dim and grid[i][j+1] != 1 and p[i][j] != (i,j+1):
            if p[i][j+1] != -1:
                if g[i][j]+1+h[i][j+1]<f[i][j+1]:
                    delete_fringe((i,j+1),fringe)
            if (p[i][j+1] == -1) or (p[i][j+1] !=-1 and g[i][j]+1+h[i][j+1]<f[i][j+1]):
                p[i][j+1]=(i,j)
                g[i][j+1]=g[i][j]+1
                f[i][j+1]=g[i][j+1]+h[i][j+1]
                insert_fringe(i,j+1,fringe,f)
            
        if i+1 < dim and grid[i+1][j] != 1 and p[i][j] != (i+1,j):
            if p[i+1][j] != -1:
                if g[i][j]+1+h[i+1][j]<f[i+1][j]:
                    delete_fringe((i+1,j),fringe)
            if (p[i+1][j] == -1) or (p[i+1][j] !=-1 and g[i][j]+1+h[i+1][j]<f[i+1][j]):
        ...
        if j-1 >= 0 and grid[i][j-1] != 1 and p[i][j] != (i,j-1):
            if p[i][j-1] != -1:
                if g[i][j]+1+h[i][j-1]<f[i][j-1]:
                    delete_fringe((i,j-1),fringe)
            if (p[i][j-1] == -1) or (p[i][j-1] !=-1 and g[i][j]+1+h[i][j-1]<f[i][j-1]):
    ...
    return(result, start, stop, p, g, h, f, order)
